...
notebooks can `from research.ner_metrics import ...` instead of redefining them.
...
def compute_iou_raw(span_a: tuple[int, int], span_b: tuple[int, int]) -> float:
    """Plain IoU between two intervals; no label/threshold gating."""
    s_a, e_a = span_a
    s_b, e_b = span_b
    if e_a <= s_b or e_b <= s_a:
        return 0.0
...
def bipartite_greedy_match(
    ...
    Each pred matches at most one gold and each gold matches at most one
    pred. This makes ``matched <= min(len(pred), len(gold))`` so aggregate
    precision/recall are always in ``[0, 1]`` — fixing the asymmetry that
    previously diverged between :func:`calculate_metrics` (which broke
    after the first match per pred) and the bootstrap counter (which
    counted every overlapping pair).

    Returns the set of matched ``(pred_idx, gold_idx)`` pairs.
    """
    candidates: list[tuple[float, int, int]] = []
    for pi, ps in enumerate(pred_spans):
        for gi, gs in enumerate(gold_spans):
            if require_label_match and ps[2] != gs[2]:
                continue
            iou = compute_iou_raw((ps[0], ps[1]), (gs[0], gs[1]))
            if iou >= iou_threshold:
                candidates.append((iou, pi, gi))
    # Highest IoU first; ties broken by (pi, gi) order for determinism.
...
def calculate_metrics(
    ...
    for _, row in df.iterrows():
        ...
        for pi, gi in bipartite_greedy_match(
            pred_spans, gold_spans, iou_threshold=iou_threshold
        ):
